...
def about():
    if 'email' in session:
        response =  requests.get("http://127.0.0.1:5000/boletos",json=session['user_id'])
        tickets = response.tickets
        events = response.events
        return render_template('about.html', title='Mis Boletos', tickets=tickets, events=events.json())
    return render_template('login.html', title='Login', form=form)
